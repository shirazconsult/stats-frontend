...
public abstract class MultipleChartPortlet extends PortletWin implements DataLoadedEventHandler, MaximizeClickHandler, RestoreClickHandler {
	...
	protected abstract DataView[] getViews();
	protected abstract CoreChart[] getCharts();
	...
	public void draw(){
		try{
			DataView[] views = getViews();
			Options[] options = getOptions();
			if(getCharts() == null || getCharts().length == 0){
//				CoreChart chart = new PieChart(dataProvider.getDataTable(), getOptions());				

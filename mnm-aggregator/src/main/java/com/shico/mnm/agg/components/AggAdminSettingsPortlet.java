...
public class AggAdminSettingsPortlet extends PortletWin implements DataLoadedEventHandler {
	...
	public AggAdminSettingsPortlet(AggSettingsController settingsConroller) {
		super(TITLE);
	...
	private DynamicForm getUserSettingsForm() {
		if(userSettingsform == null){
			...
			TextItem userItem = new TextItem(AggRemoteSettingsDS.AGGREGATORUSER, "User name");
			TextItem pwdItem = new PasswordItem(AggRemoteSettingsDS.AGGREGATORPWD, "Password");
...
//			pwdItem.setRequired(true);
	...
	private DynamicForm getAdminSettingsForm() {
		if(adminSettingsform == null){
			...
			TextItem urlItem = new TextItem(AggRemoteSettingsDS.AGGREGATORURL, "Aggregator Manager Url");
				...
				valuesManager.saveData(new Callback<Map, String>() {					
					...
					public void onSuccess(Map result) {
						if(result != null){
							...
							EventBus.instance().fireEvent(new DataLoadedEvent(DataEventType.AGG_ADMIN_SETTINGS_CHANGED_EVENT, result));
						}else{

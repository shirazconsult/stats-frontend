...
//					arrowFormat();
	...
	private native DataView getNativeEventProcessingTimeView(DataView view, DataTable data)/*-{
		view.setColumns([
			...
			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::numOfTotalExchangesIdx, label:'Number of Exchanges'},
			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::totalProcessingTimeIdx, label:'Total Processing Time'},
			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::lastProcessingTimeIdx, label:'Last Exchange Processing Time'},
			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::maxProcessingTimeIdx, label:'Max Processing Time'},
			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::minProcessingTimeIdx, label:'Min Processing Time'},
			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::meanProcessingTimeIdx, label:'Mean Processing Time'}
			]);
		
		function toTimeWindow(dataTable, rowNum){
			$wnd.twim = @com.shico.mnm.agg.client.AggregatorDataClient::timeSliceInMinutes()();
			var cursor = @com.shico.mnm.agg.client.AggregatorDataClient::slidingCur;			
	...
	private native DataView getNativeLoadView(DataView view, DataTable data)/*-{
		view.setColumns([
						{calc:toTimeWindow, type:'string', label:'Time'},
		     			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::lastMinuteLoadIdx, label:'Last Minute Load'},
		     			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::last5MinuteLoadIdx, label:'Last 5 Minutes Load'},
		     			{sourceColumn:@com.shico.mnm.agg.client.AggregatorChartDataProvider::last15MinuteLoadIdx, label:'Last 15 minutes Load'}
		     			]);
	...
	public void schedule(final int schedulePeriod, int viewWindow) {
		...
		timeSlice = (slidingWinTime / 5) * 60000; // each sliding window will contain up to five data-rows
		...
		slidingWinRows = (int)((slidingWinTime * 60000) / timeSlice);
		vmSlidingWinRows = (slidingWinTime * 60) / scheduleIntervalSec;
	}
...
//			String chartUrl = (String)settingsController.getSetting(AdminSettingsDS.CHARTURL);
